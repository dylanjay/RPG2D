...
namespace Benco.BehaviorTree.TreeEditor
{
    public class NodeGraph : ScriptableObject
    {
        ...
        //TODO: Does this need serialization?
        [SerializeField, HideInInspector]
        private NodeBase connectionNode;
        ...
        public void DeleteNode(NodeBase node)
        {
            ...
            // If the node has children, remove the children's parent references.
            if (node.output != null)
            {
                foreach (NodeBase child in node.output.childNodes)
                {
                    Debug.Assert(child.input.parentNode == node, "Child node didn't point to the the correct parent.");
                    child.input.parentNode = null;
                }
                node.output.childNodes.Clear();
            }

            if (node.input != null)
            {
                // Check out this sweet line of code!
