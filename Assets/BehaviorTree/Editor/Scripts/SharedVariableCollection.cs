...
namespace Benco.BehaviorTree
{
    ...
    public class SharedVariableCollection : ISerializationCallbackReceiver
    {
        ...
        /// <summary>
        /// All possible dropdowns. The key is the full name type of the object, the value is the list of GUIContent options (SharedVariables).
        /// </summary>
        [SerializeField]
        ...
        public void AddVariable(string name, Type type)
        {
            ...
            if (name == none.name)
            {
                Debug.LogError("For confusion reasons with the GUI, naming your variable \"None\" has been disabled. Please choose a more verbose name.");
                return;
            }
            ...
            Type sharedVarType = System.Array.Find(NodeUtilities.sharedVariableDerivedTypes, findSharedType);
        ...
        public void RemoveVariable(string name, Type type)
        {
            ...
            {
                Debug.LogWarning(string.Format("There nodes that are still currently using SharedVariable \"{0}\". These references will be cleared.", removedSharedVariable.name));
                foreach (NodeFieldPair reference in references[removedSharedVariable])
                {
                    NodeBase node = reference.node;
            ...
            foreach (FieldInfo fieldInfo in behaviorType.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
            {
                    ...
                    (((Attribute[])fieldInfo.GetCustomAttributes(typeof(HideInInspector), true)).Length != 0 && fieldInfo.IsPublic)))
                {
                    ...
                    else
                    {
                        ...
                        GUIContent guiContent = dropdowns[varType.FullName].ToList().Find(x => x.text == fieldInfo.Name);
